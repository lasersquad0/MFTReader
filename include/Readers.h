...
public:
	...
	TErrorCode GetPathByMFTRecID(MFT_REF mftRecRef, THArray<std::wstring>& paths);
	std::expected<MFTRecIndex, TErrorCode> GetMFTRecIdByPath(const ci_string& path); // ci_string is for case INsensitive search here
};
...
public:
	...
	TErrorCode ReadMftItems(MFT_REF startMftRecRef, uint32_t dirLevel);
